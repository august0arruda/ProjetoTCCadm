        ...
        if (Auth::checkAuthNoform($_POST) === true)//verifica se a chave JWT.io e válida sem o uso de formulario
        {
            ...
            switch ($metodo_)
            {
                case 'create':
                    ...
                    if(!isset($metodo_id)){return Cidadao::insert($form_data);}else{return self::$fail_metodo;}
                    break;
                case 'reader':
                    //172.17.101.255/ProjetosNAP/sapopemba-telessaude/App_public/api/user/reader=21000001
                    if(!isset($metodo_id)){return Cidadao::selectAll();}else{return Cidadao::select($metodo_id);}
                    break;
                ...
                case 'usercreate_consumer':
                    //172.17.101.255/ProjetosNAP/sapopemba-telessaude/App_public/api/user/usercreate_consumer=admin
                    if(!isset($metodo_id)){return self::$fail_metodo;}else{return Cidadao::usercreate_consumer($metodo_id);}
                    break;
